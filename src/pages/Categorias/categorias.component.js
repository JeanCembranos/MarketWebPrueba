...
class Categorias extends Component {
  ...
  onCollectionUpdate = (querySnapshot) => {
    ...
    querySnapshot.forEach((doc) => {
      console.log("Inside");
      console.log(doc);
      const { gridimage,key,name } = doc.data();
      boards.push({
        gridimage,
        key,
        name
      ...
      <div>
        ...
        <div className='container'>
          ...
          <div className='panel-body'>
            <h4>
              <RouterLink to='/createcateg' className='btn btn-primary '>Añadir Categoría
              </RouterLink>
            </h4>
            <input type="text" id="myInput" onChange={this.myFunction}
          placeholder="Busca la categoría" title="Type in a name">
          </input>
              ...
              <tbody>
                {
                  this.state.filterboards.map(board =>
                    <tr>
                        <td>{<img
                      className="ref"
                      src={board.gridimage || "https://via.placeholder.com/400x300"}
                      alt="Uploaded Images"
                      height="75"
                      class="center"
                    />}</td>
